...
# -------- increment GIF path --------
def next_gif_path(base: str = 'suolson_evolution') -> str:
...
def gmres_callback(rk):
    residuals.append(rk)
    print(f"  GMRES iter {len(residuals):3d}: residual = {rk:.3e}")
...
def init_plot(U0, V0, xi, xf, yi, yf, bc_x, bc_y):
    ...
    # U_vmax   = max(float(jnp.max(jnp.abs(U0))),   1e-6)
    # V_vmax   = max(float(jnp.max(jnp.abs(V0))),   1e-6)
    # tot_vmax = max(float(jnp.max(jnp.abs(tot0))), 1e-6)
    U_vmax  = 0.5
    V_vmax = 0.35
    tot_vmax = 0.7
...
def update_plot(img_U, img_V, img_tot, ax_U, ax_V, ax_tot, U, V, step, bc_label, tau, displayPlot=True):
    ...
    # Dynamic clims: solution grows from zero, so update every frame
    # U_vmax   = max(float(jnp.max(U)),   1e-10)
    # V_vmax   = max(float(jnp.max(V)),   1e-10)
    # tot_vmax = max(float(jnp.max(tot)), 1e-10)
    U_vmax  = 0.5
    V_vmax = 0.35
    tot_vmax = 0.7
...
def save_gif(frames, path='rd_evolution_001.gif', fps=6):
    if not frames:
...
def runSimulation(PRECISION,
    ...
    gif_path = next_gif_path(f'{figFolder}/rd_evolution')
    print(f"Output GIF: {gif_path}")

    # ---- grids --------------------------------------------------------
    # Symmetric domain [-L, L] in x: vacuum BCs at both ends, wave symmetric about x=0.
    # y is homogeneous (Su-Olson is 1D in x, embedded on a 2D grid for generality).
    xi, xf   = -5.0, 5.0
    yi, yf   = -5.0, 5.0
    ...
    def _scipy_matvec_fn(U_k, V_k, Nx, Ny, p, dt, epsilon, Q, dx, dy, bc_x, bc_y):
        # For the FD Jacobian: pass U_k / V_k as the "old" values.
        # The old-time terms cancel exactly in F(w+eps*p) - F(w), so this is correct.
    ...
    for step in range(steps):
        ...
        for k in range(NewtonIter):
            ...
            # --- Unified SciPy GMRES (FD or AD Jacobian) ---
            if useAD:
                JLinearOp = LinearOperator(
                    (2*Nx*Ny, 2*Nx*Ny),
                    matvec=lambda vec: np.asarray(
                        JacobianActionAD_jit(
                            U_k, V_k, U_old, V_old, jnp.array(vec, dtype=dtype),
                            dt, epsilon, Q, dx, dy, BC_X, BC_Y, Nx, Ny
                        )
                    ).copy()
                )
            else:
                JLinearOp = LinearOperator(
                    (2*Nx*Ny, 2*Nx*Ny),
                    matvec=lambda vec: np.asarray(
                        JacobianActionScipy_jit(
                            U_k, V_k, Nx, Ny, jnp.array(vec, dtype=dtype),
                            dt, epsilon, Q, dx, dy, BC_X, BC_Y
                        )
                    ).copy()
                )
            ...
            residuals = []
            if verbose:
                delta_state, info = gmres_scipy(
                    JLinearOp, -np.asarray(F_vec).copy(),
                    rtol=KrylovTol, maxiter=KrylovIter, callback=gmres_callback
                )
                print(f"  SciPy GMRES info: {info}")
            else:
                delta_state, info = gmres_scipy(
                    JLinearOp, -np.asarray(F_vec).copy(),
                    rtol=KrylovTol, maxiter=KrylovIter
                )

            # ---- Backtracking line search to stabilise Newton near strong gradients -----
            delta_U, delta_V = flattenJnp(jnp.array(delta_state, dtype=dtype), Nx, Ny)
            ...
            for ls in range(maxBackTrackingIter):
                ...
                # Apply vacuum BCs to radiation field; V has no spatial BC but
                # we enforce consistency at the boundary for cleanliness.
...
--- Overall Time, s
  ...
  Physical Time Simulated : {tau:.4f}
{"="*50}
"""
...
if __name__ == "__main__":
    ...
    # Set to 'CLASSIC_SU_OLSON' or '2D_DYNAMIC_BEAUTY'
    SIMULATION_TYPE = '2D_DYNAMIC_BEAUTY'
    ...
    elif SIMULATION_TYPE == '2D_DYNAMIC_BEAUTY':
    ...
    gif_fps         = 10
    displayPlot     = True
    figFolder       = "output/radiationdiff"
