...
from bicgstabCuPy import bicgstab                # Custom CuPy-ish GPU BiCGSTAB solver (soon in CuPy)
    ...
    for step in range(steps):
        ...
        for k in range(NewtonIter):
            ...
            if KrylovSolver.lower() == 'gmres':
                ...
                # Call your custom CuPy BiCGSTAB implementation!
                delta_vel_cp, info = bicgstab(
                    JLinearOp, F_vec_cp, rtol=KrylovTol, maxiter=KrylovIter, callback=krylov_counter
                )
            elif KrylovSolver.lower() == 'cgs':
                # Use Conjugate Gradient Squared instead of BiCGSTAB
                delta_vel_cp, info = cupy_spla.cgs(
                    JLinearOp, F_vec_cp, rtol=KrylovTol, maxiter=KrylovIter, callback=krylov_counter
                )
            else:
                raise ValueError("KrylovSolver must be 'gmres' or 'cgs'")
...
if __name__ == "__main__":
    ...
    dataFolder      = f"data/{SIMULATION_IC}_GPU_{Nx}_{Ny}_{'AD' if useAD else 'FD'}_{PRECISION}"
