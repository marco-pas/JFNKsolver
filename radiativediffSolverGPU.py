...
def runSimulation(PRECISION, BC_X, BC_Y, SIMULATION_IC, SOURCE_TYPE, verbose, useAD, maxBackTrackingIter,
                  epsilon, Q0, x_src, tau_src, Courant, steps, Nx, Ny,
    ...
    for step in range(steps):
        ...
        for k in range(NewtonIter):
            ...
            if useAD:
                ...
                def A_matvec_jax(vec):
                    return JacobianActionFD_jit(U_k, V_k, F_U_k, F_V_k, Nx, Ny, vec, dt, epsilon, Q, dx, dy, BC_X, BC_Y)
        ...
        if (step + 1) % plot_steps == 0:
            update_plot(img_U, img_V, img_tot, ax_U, ax_V, ax_tot, U, V, step + 1, bc_label, tau, displayPlot)
        ...
        if save_steps > 0 and (step + 1) % save_steps == 0:
            ...
            # Save files (e.g., data/U_step0100_t3.000000.npy)
            U_path = f"{dataFolder}/U_step{step + 1:04d}_t{tau:.6f}.npy"
            V_path = f"{dataFolder}/V_step{step + 1:04d}_t{tau:.6f}.npy"
...
if __name__ == "__main__":
    ...
    Courant         = 10              # fixed timestep (backward Euler: unconditionally stable)
